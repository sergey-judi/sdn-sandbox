...
def topology(test):
    ...
    c1 = net.addController(name="c1", controller=RemoteController, protocol="tcp", ip=CONTROLLER_IP, port=6653)

    info("*** Adding switches\n")
    s1 = net.addSwitch("s1", cls=OVSSwitch, dpid="00:00:00:00:00:06", dpopts='', protocols="OpenFlow13")		
    ...
    # Run specified test
    if test == 0:
        test0.run_test(net, CONTROLLER_IP)
    elif test == 1:
        test1.run_test(net, CONTROLLER_IP)
    elif test == 2:
        test2.run_test(net, CONTROLLER_IP)
    elif test == 3:
        test3.run_test(net, CONTROLLER_IP)
